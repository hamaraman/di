...
VLOOKUP을 쓰다 보면 값이 분명히 있는데도 `#N/A` 오류가 뜨는 경우가 있습니다. 원인은 대부분 정해져 있어서, 아래 순서대로 확인하면 거의 해결됩니다.
...
- 해결: 텍스트를 숫자로 바꾸려면 `=VALUE(A2)`, 반대로 숫자를 텍스트로 맞추려면 `=TEXT(A2,"0")` 를 사용해 형식을 통일합니다.
